...
点击浏览器工具栏的 Mouse Bridge 图标**直接打开浏览器侧栏**（Side Panel，
整个右侧区域、不会因失焦而关闭）；快捷键 `Alt+Shift+M`。Edge 部分版本对
侧栏扩展不派发图标点击事件，此时用原生入口：右键工具栏图标 →「打开边栏」，
或扩展菜单（拼图图标）→「在侧边栏中打开」。

面板功能（卡片式布局）：

- **连接**：守护进程运行/版本、扩展 WS 连接状态；守护进程未运行时提供
  「启动守护进程」按钮；
- **实时坐标**：开关开启后每个页面右下角显示 `css(x,y) dev(x,y)`
  双坐标（与测试页同款），移动鼠标即刷新，供你读取任意位置的页面坐标；
- **移动光标**：把读到的 X/Y 填进去点「移动」，守护进程按页面视口 css 坐标
  换算（走同一套仿射标定）并以拟人轨迹移动真实光标，面板回显页面确认的
  落点残差。面板发出的移动请求固定路由到**面板所在的那个浏览器**。

### 多浏览器同时连接（v1.2.2）

Chrome 和 Edge 可以**同时**连着守护进程，互不挤占。路由规则：

- 请求带 `ext_id`（`扩展ID@浏览器`，侧栏面板自动带上自己的）→ 精确路由到
  那个浏览器；
- 请求带 `url`/`tab_id` → 逐个浏览器尝试，「这个浏览器没有这个标签页」的
  回答自动落到下一个；
- 什么提示都没有 → 最近使用/最新连接的浏览器优先。
- 扩展定位过某个页面后，后续测量固定走同一个浏览器（tab id 只在单个浏览器
  内唯一，跨浏览器会撞号）。
- `GET /status` 的 `extensions` 数组列出每个连接（身份、版本、caps）。

### 测量标定 v2：中间区域 + 自纠偏（v1.2.2）

三个测量点全部落在页面**中央 30%–70% 区域**，这样位置预估即使偏差几百像素
（侧栏停靠、开发者工具展开、特殊窗框）也仍在页面内可测。第一次读到页面的
实话后立即用这一对读数**修正后续瞄准**（缩放比例独立已知，一对数据即可锁定
偏移）；某个点页面看不见（被浏览器 UI 遮挡）就**换点重试**而不是报错放弃；
「点错窗口」防护保留在修正之后——修正后仍对不上才中止。实测：侧栏打开时
...
bin/trajlab.exe pick -dist 500 -angle 0      # 从合格真人轨迹里随机抽一条（±10% 距离，方向近者优先）
...
拷贝到其他机器）。建议按 50px 网格录制；距离 ±10% 内的录制都算合格，超出则算法生成。

### 守护进程优先回放真人轨迹（v1.2.4）

守护进程收到移动指令后先查真人库，**选路规则**：

- **±10% 距离合格**：以录制本身的长度为基准——100px 的录制可以服务 90~110px
  的移动（105px 的录制可服务 94.5~115.5px），超出范围才用算法生成；
- **随机挑选**：合格的多条里随机抽一条，不做"最接近就精确配对"，同距同向
  重复请求也不会总回放同一条；
- **方向对得上优先**：合格且方向差 ≤45° 的优先入池；库里没有对得上的方向时
  （如只有向右/向上的录制，却要向左移），从全部合格里随机抽并把整条轨迹
  旋转到目标方向；
- **等比缩放**：选中的轨迹整体平移+旋转+等比缩放映射到本次起终点，**点数和
  每个点的时间戳不变**，终了精确落在目标点上——录制时的真实手速、弧线、
  抖动原样保留，只是几何按比例放大缩小。

响应里新增 `source` 字段（`human:<文件名>` 或 `synthetic`），守护进程日志同步
记录每次移动用了哪条轨迹。拖拽仍走专用合成模型（带按住的比例感），移动路径
不变。库为空或没有合格录制时行为与 v1.2.3 完全一致。
...
cmd/trajlab/          轨迹实验室 CLI（验收矩阵/真人录制/回放/真人轨迹挑选）
